...
function materializeLayoutHeader(page, layout) {
  ...
  // Header recovery already has explicit x-ranges for each header group.
  // Rebuild from those groups instead of re-querying page.words by Y. This
  // avoids a subtle failure where a nearby first-data line is included in a
  // boundary band and gets concatenated into the header cell.
  const groups = header.groups || [];
  if (!groups.length) return null;

  const cells = layout.columns.map(() => ({
    text: "",
    colspan: 1,
    rowspan: 1
  }));
  ...
  for (const group of groups) {
    ...
    for (let i = 0; i < layout.columns.length; i++) {
      ...
      if (center >= column.x0 - 2 && center <= column.x1 + 2) {
        columnIndex = i;
        break;
      }
    }
    ...
    const text = normalizeText((group.words || []).map(word => word.text).join(" "));
    if (!text) continue;

    cells[columnIndex].text = cells[columnIndex].text
      ? `${cells[columnIndex].text} ${text}`
      : text;
  }

  if (!cells.some(cell => cell.text)) return null;

  const headerWords = groups.flatMap(group => group.words || []);

  return {
    cells,
    top: header.y0,
    bottom: header.y1,
    bold: headerWords.filter(w => /bold|semibold|demi/i.test(String(w.fontFamily || ""))).length >= Math.ceil(headerWords.length * 0.35),
    fontSize: median(headerWords.map(w => Number(w.fontSize) || 0).filter(Boolean)) || 0,
...
function isHeaderContinuationRow(row, header, columnCount) {
  ...
  // A populated first column that looks like a product/code ID is usually
  // the first real data row, not a second header line. Multi-row headers may
  // contain short alphanumeric labels, but should not look like a complete
  // data row across most columns.
  if (codeLike && filled >= Math.max(3, Math.ceil(columnCount * 0.75))) return false;
